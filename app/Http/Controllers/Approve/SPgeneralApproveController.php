...
use App\Mail\ApproveSuperUser;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class SPgeneralApproveController extends Controller
{
    public function process(Request $request, $id)
    {
        $userrequest = GeneralRequest::withTrashed()->find($id);
        $userrequest->state = $request->get('state');
        $userrequest->assign_id = $request->get('assign_id');
        ...
        $comment->content = "Approve and" . $request->get('content');
...
//            \Mail::to(['it@hayman-capital.com'],"User-Request")->send(new ApproveSuperUser($data ));
